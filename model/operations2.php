...
function connect()
{
   return mysqli_connect("localhost", "angacinemas_hello", "6wVCtmoa2CoI", "angacinemas_hello");
}
...
function build_multiple_sql_edit($table, $elements, $id, $ids,$cols,$col = "id")
{
    $sql = "update `$table` set ";
    $i = sizeof($elements);
    $j = 0;
...
<div class="form-group">
    <label><?php echo $label ?></label>
    <textarea class="form-control" style="min-width: 100%; max-width: 100%;" name="<?php echo $name ?>" <?= $readonly == true ? 'readonly="readonly" ' : '' ?> id="<?php echo $name ?>" rows="5" <?php echo $required ? 'required' : '' ?>><?php echo !empty($row) ? $row[$name] : '' ?></textarea>
</div>
<?php
}
...
function upload($name2, $type = 'img')
{
    if (isset($_FILES[$name2]["name"]) && $_FILES[$name2]["name"] != '') {
        ...
        $target_dir = $_SERVER["DOCUMENT_ROOT"]."/backoffice/uploads/";
...
function uploadz($name2, $type = 'img')
{
    if (isset($_FILES[$name2]["name"]) && $_FILES[$name2]["name"] != '') {

        $target_dir = "../backoffice/uploads/";
        ...
        $tmpname = $_FILES[$name2]["tmp_name"];
        $extention = strtolower(pathinfo($name, PATHINFO_EXTENSION));
        $filename = ($type != 'img' ? 'vid' : 'img') . $rand . date("yhjHidYsma") . $rands;
        $image_path = $target_dir . $filename . "." . $extention;

        $check = $type != 'img' ? check_ext_vid($extention) : check_ext($extention);
...
function limit_text($content, $limit  = 8)
{
    $text   = $content;
...
function limit_textz($content, $limit  = 3)
{
    $text   = $content;
    if (str_word_count($text, 0) > $limit) {
        $words = str_word_count($text, 2);
...
function encryptString($string, $action, $encryptedIP = '')
{
    ...
    if(isset($flag['2nd-encrypt-secret'] )){
