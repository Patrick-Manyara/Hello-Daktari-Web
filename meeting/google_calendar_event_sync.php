...
if (isset($_GET['code'])) {
    ...
    if (!empty($event_id)) {
        ...
        $resultData = select_rows("SELECT * FROM events WHERE id = $event_id");
        ...
        if (isset($eventData) && !empty($eventData)) {
            ...
            if (!empty($access_token)) {
                try {
                    ...
                    $google_event = $GoogleCalendarApi->CreateCalendarEvent($access_token, 'primary', $calendar_event, 0, $event_datetime, $user_timezone);
                    ...
                    if (isset($google_event) && isset($google_event["id"])) {
                        ...
                        $build_edit = build_sql_edit('events', array('google_calendar_event_id' => $google_event_id), $event_id, "id");
